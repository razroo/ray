...
function validateDeployWorkflowStoragePreflight(
  ...
  if (
    ...
    contents.includes("/srv/ray/scripts/deploy-storage-preflight.ts") &&
    contents.includes("--ray-env-file /etc/ray/ray.env") &&
...
function validateDeployWorkflowGatewayRuntimeBunInstall(
  ...
  if (
    ...
    contents.includes(
      'if ! timeout 60s $SUDO install -m 0755 "$bun_install_dir/bin/bun" /usr/local/bin/bun; then',
    ) &&
    contents.includes("Could not install Bun to /usr/local/bin/bun.") &&
