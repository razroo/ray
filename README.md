...
state, `/var/log`, `/tmp`, `/var/tmp`, and repo-scoped Bun install-cache volumes
before package bootstrap, rsync follow-up, config/unit/Caddy writes, journal
output, or the remote Bun production install.
Doctor uses the same value for strict `/var/log`, `/tmp`, and `/var/tmp`
journal/log and temporary-storage headroom and for the generated
`WorkingDirectory` storage cushion, with a 512 MiB floor for the synced checkout
and Bun production install.
