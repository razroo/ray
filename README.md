...
That runs lint, Prettier `--check`, tests (`bun run test` builds, runs the compiled Tap suite, then runs script tests with Bun), all checked-in config validation, public deploy bundle smoke rendering, public model staging smoke rendering, public deploy package-script coverage, Bun-first package, workflow, and runtime-doc coverage, and Bun pack smoke checks for the public packages.
