...
Gateway startup binds the HTTP listener before provider warmup finishes, so `/livez` can keep systemd and reverse proxies pointed at the running process while `/readyz` returns 503 until the provider is available and warmed, then reports backend-aware readiness plus minimal async-queue degradation. On shutdown the gateway stops accepting new HTTP sockets and force-closes lingering active connections after 30 seconds, leaving the generated `TimeoutStopSec=35` unit enough room to restart cleanly even when a slow client is stuck.
