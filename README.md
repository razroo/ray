...
more or less than the default 1024 MiB free on the root, `/etc/ray`,
`/etc/systemd/system`, `/etc/caddy`, checkout, Ray state, `/tmp`, `/var/tmp`,
and repo-scoped Bun install-cache volumes before package bootstrap, rsync
follow-up, config/unit/Caddy writes, or the remote Bun production install. When
`--ray-env-file` is supplied, the same preflight also checks custom
`RAY_MODEL_PATH` or `RAY_LLAMA_CPP_MODEL_PATH` and
