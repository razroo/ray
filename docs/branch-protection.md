...
After this, contributors cannot merge PRs until lint, format check, tests, all checked-in config validation, public deploy bundle smoke rendering, public model staging smoke rendering, and Bun pack smoke checks match what `bun run release:gate` runs locally.
