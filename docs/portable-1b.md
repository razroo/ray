...
Doctor checks auth/env readiness, `llama-server` executable readiness, model file presence, launch profile consistency, projected memory fit against the selected memory budget, async queue storage headroom, and swap cushion for the 4 GB llama.cpp profile before the service starts.
