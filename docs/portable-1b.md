...
Doctor checks auth/env readiness, env-file permissions, generated systemd user readiness, service-user access to the rendered Bun runtime (`/usr/local/bin/bun` by default, or `--gateway-runtime-binary`), generated WorkingDirectory, built gateway entrypoint, `llama-server` binary, GGUF model file, and async queue storage, launch profile consistency, projected memory fit against the selected memory budget, async queue storage headroom, and swap cushion for the 4 GB llama.cpp profile before the service starts.
