...
- Keep `/tmp` and `/var/tmp` with the same storage cushion before running doctor, render validation, Caddy validation, fallback Bun installs, or staging helpers; doctor checks both temporary storage paths because those workflows create bounded temp files even when final artifacts land elsewhere.
