...
- Keep `RAY_REQUEST_BODY_LIMIT_BYTES` close to the largest prompt payload the workload actually needs. Doctor warns when the body cap can retain too much admitted upload data against the generated gateway `MemoryMax`, and the gateway rejects oversized `Expect: 100-continue` uploads before acknowledging the body.
