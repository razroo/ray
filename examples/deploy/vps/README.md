...
- Use `model.adapter.launchProfile.extraArgs` only for non-profile llama.cpp flags such as template or mmap tuning. Config loading and the systemd renderer reject extra args that override generated host, port, model path, alias, context, batching, thread, cache, slot, metrics, or warmup settings; command-line args take priority over llama.cpp environment defaults, so those settings must stay in first-class config or `RAY_LLAMA_CPP_*` env overrides.
