...
`node_modules`.
The storage preflight checks `/etc/ray`, `/etc/systemd/system`, `/etc/caddy`,
`/srv/ray`, `/var/lib/ray`, `/tmp`, `/var/tmp`, and the repo-scoped Bun install
cache at `/srv/ray/.ray/bun-install-cache` before the Bun install expands
dependencies or deploy writes config, units, and the Caddyfile; set
...
`/var/lib/ray`, `/tmp`, `/var/tmp`, and repo-scoped Bun install-cache paths
before bootstrap follow-up, config/unit/Caddy writes, or Bun production install can consume more disk,
