...
- Keep `RAY_REQUEST_BODY_LIMIT_BYTES` close to the largest prompt payload the workload actually needs. Doctor warns when the body cap can retain too much admitted upload data against the generated gateway `MemoryMax`, and the gateway acknowledges `Expect: 100-continue` uploads only after auth, rate-limit, content-type, and declared-size admission pass.
