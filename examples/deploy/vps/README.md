...
- Ray also samples Linux cgroup memory files when available, marks memory pressure when the service or container reaches 90% of its configured cgroup memory limit, and exposes process RSS pressure ratio, cgroup swap usage from v2 `memory.swap.*` or legacy v1 `memory.memsw.*` files, plus cgroup v2 `memory.events` counters in health and metrics so operators can see when swap, `MemoryHigh`, or OOM boundaries were crossed.
