...
- `RAY_DEPLOY_MIN_FREE_STORAGE_MIB` — minimum free storage the workflow requires before remote APT package bootstrap, checkout sync follow-up, config/unit/Caddy writes, Bun install-cache use, Bun production install, and env-file model, llama.cpp binary, or async-queue storage use, defaults to `1024`; doctor also uses it for the generated `WorkingDirectory` storage cushion with a 512 MiB floor, and `0` only skips the deploy storage preflight threshold
...
root, `/etc/ray`, `/etc/systemd/system`, `/etc/caddy`, `/srv/ray`,
`/var/cache/apt`, `/var/lib/apt`, `/var/lib/ray`, `/tmp`, `/var/tmp`, and
...
- Keep `RAY_REQUEST_BODY_LIMIT_BYTES` close to the largest prompt payload the workload actually needs. Doctor warns when the body cap can retain too much admitted upload data against the generated gateway `MemoryMax`, the gateway acknowledges `Expect: 100-continue` uploads only after auth, rate-limit, content-type, and declared-size admission pass, and unsupported `Expect` values receive bounded JSON `417` rejects.
