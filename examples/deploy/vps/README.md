...
- Detailed `/health` exposes provider-preparation, durable async-queue, gateway HTTP socket pressure and limits, queue depth, in-flight work, queued-token, and in-flight-token saturation ratios so operators can see admission pressure without a separate metrics scrape.
- The `/metrics` endpoint refreshes live provider-preparation, queue, cache, async-job saturation, async storage headroom, gateway HTTP socket saturation and threshold gauges, and cgroup resource gauges before responding, so a scraper can observe current pressure without a separate health probe.
